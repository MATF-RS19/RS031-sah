...
void Chess::figuraNapadaPolja(int u,bool* a){
    ...
    if(f->Oznaka()==QOznaka::pijun && f->Boja()==QBoja::bela){
        if(u>7 && !mapa.contains(u-8))
            a[u-8]=true;
        if(mapa.contains(u-7) && mapa[u-7]->Boja()==QBoja::crna)
            a[u-7]=true;
        if(mapa.contains(u-9) && mapa[u-9]->Boja()==QBoja::crna)
            a[u-9]=true;
        if(brojPoteza==0)
            a[u-16]=true;
    }
        ...
        if(u+8<64 && !mapa.contains(u+8))
            a[u+8]=true;
        if(mapa.contains(u+7) && mapa[u+7]->Boja()==QBoja::bela)
            a[u+7]=true;
        if(mapa.contains(u+9) && mapa[u+9]->Boja()==QBoja::bela)
            a[u+9]=true;
        if(brojPoteza==1)
            a[u+16]=true;
    }
        ...
        if(u-16-1>-1 && (!mapa.contains(u-16-1) || mapa[u-17]->Boja()!=f->Boja()))
            a[u-17]=true;
        if(u-16+1>-1 && (!mapa.contains(u-16+1) || mapa[u-15]->Boja()!=f->Boja()))
            a[u-15]=true;
        if(u+16-1<64 && (!mapa.contains(u+16-1) || mapa[u+15]->Boja()!=f->Boja()))
            a[u+15]=true;
        if(u+16+1<64 && (!mapa.contains(u+16+1) || mapa[u+17]->Boja()!=f->Boja()))
            a[u+17]=true;
        if(u-8-2>-1 && (!mapa.contains(u-8-2) || mapa[u-10]->Boja()!=f->Boja()))
            a[u-10]=true;
        if(u-8+2>-1 && (!mapa.contains(u-8+2) || mapa[u-6]->Boja()!=f->Boja()))
            a[u-6]=true;
        if(u+8-2<64 && (!mapa.contains(u+8-2) || mapa[u+6]->Boja()!=f->Boja()))
            a[u+6]=true;
        if(u+8+2<64 && (!mapa.contains(u+8+2) || mapa[u+10]->Boja()!=f->Boja()))
            a[u+10]=true;
    }
    else if(f->Oznaka()==QOznaka::lovac){
        int i;
        for(i=1;u-9*i>-1 && !mapa.contains(u-9*i);++i)
            a[u-9*i]=true;
        if(u-9*i>-1 && mapa[u-9*i]->Boja()!=f->Boja())
            a[u-9*i]=true;

        for(i=1;u-7*i>-1 && !mapa.contains(u-7*i);++i)
            a[u-7*i]=true;
        if(u-7*i>-1 && mapa[u-7*i]->Boja()!=f->Boja())
            a[u-7*i]=true;

        for(i=1;u+9*i<64 && !mapa.contains(u+9*i);++i)
            a[u+9*i]=true;
        if(u+9*i<64 && mapa[u+9*i]->Boja()!=f->Boja())
            a[u+9*i]=true;

        for(i=1;u+7*i<64 && !mapa.contains(u+7*i);++i)
            a[u+7*i]=true;
        if(u+7*i<64 && mapa[u+7*i]->Boja()!=f->Boja())
            a[u+7*i]=true;
    }
...
bool Chess::figuraNapadaPolja(int u){
    bool* a=new bool[64];
    for(int i=0;i<64;++i)
        a[i]=false;
    figuraNapadaPolja(u,a);
    for(int i=0;i<64;++i)
        if(a[i]){
            delete[]a;
            return true;
...
void Chess::priprema(){
    if(stanje==QStanje::beliIgrac_pocetakPoteza){
        for(QMap<int,Figura*>::const_iterator i=mapa.cbegin();i!=mapa.cend();++i)
            if(i.value()->Boja()==QBoja::bela && movable(i.key()))
                intToButton(i.key())->setEnabled(true);
    }
    else if (stanje==QStanje::crniIgrac_pocetakPoteza){
        for(QMap<int,Figura*>::const_iterator i=mapa.cbegin();i!=mapa.cend();++i)
            if(i.value()->Boja()==QBoja::crna && movable(i.key()))
                intToButton(i.key())->setEnabled(true);
    }
}
...
bool Chess::movable(int u){
    ...
    if(f->Oznaka()==QOznaka::pijun && f->Boja()==QBoja::bela){
        ...
        if(u-9>-1 && !mapa.contains(u-9))
            return true;
        if(u-9>-1 && mapa[u-9]->Boja()!=f->Boja())
            return true;

        if(u-7>-1 && !mapa.contains(u-7))
            return true;
        if(u-7>-1 && mapa[u-7]->Boja()!=f->Boja())
            return true;

        if(u+9<64 && !mapa.contains(u+9))
            return true;
        if(u+9<64 && mapa[u+9]->Boja()!=f->Boja())
            return true;

        if(u+7<64 && !mapa.contains(u+7))
            return true;
        if(u+7<64 && mapa[u+7]->Boja()!=f->Boja())
            return true;
    }
...
void Chess::potez(int u){
    if(stanje==QStanje::beliIgrac_pocetakPoteza){
        ...
        if(u!=pKlik){
            ...
            intToLabel(u)->clear();
            if(!mapa.contains(pKlik))
                qFatal("mapa nema pKlik");
            crtaj(mapa[pKlik],intToLabel(u));
...
void Chess::on_pushButton_00_clicked(){potez(0);}
void Chess::on_pushButton_01_clicked(){potez(1);}
void Chess::on_pushButton_02_clicked(){potez(2);}
void Chess::on_pushButton_03_clicked(){potez(3);}
void Chess::on_pushButton_04_clicked(){potez(4);}
void Chess::on_pushButton_05_clicked(){potez(5);}
void Chess::on_pushButton_06_clicked(){potez(6);}
void Chess::on_pushButton_07_clicked(){potez(7);}
...
void Chess::on_pushButton_56_clicked(){potez(56);}
void Chess::on_pushButton_57_clicked(){potez(57);}
void Chess::on_pushButton_58_clicked(){potez(58);}
void Chess::on_pushButton_59_clicked(){potez(59);}
void Chess::on_pushButton_60_clicked(){potez(60);}
void Chess::on_pushButton_61_clicked(){potez(61);}
void Chess::on_pushButton_62_clicked(){potez(62);}
void Chess::on_pushButton_63_clicked(){potez(63);}
