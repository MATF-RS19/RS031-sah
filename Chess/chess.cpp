...
void Chess::changeButtons(int u){
    ...
    if(f->Oznaka()==QOznaka::pijun && f->Boja()==QBoja::bela){
        ...
        int i;
        for(i=1;u-9*i>-1 && u%8!=0 && !mapa.contains(u-9*i);++i)
            intToButton(u-9*i)->setEnabled(true);
        if(u-9*i>-1 && u%8!=0 && mapa[u-9*i]->Boja()!=f->Boja())
            intToButton(u-9*i)->setEnabled(true);

        for(i=1;u-7*i>-1 && (u+1)%8!=0 && !mapa.contains(u-7*i);++i)
            intToButton(u-7*i)->setEnabled(true);
        if(u-7*i>-1 && (u+1)%8!=0 && mapa[u-7*i]->Boja()!=f->Boja())
            intToButton(u-7*i)->setEnabled(true);

        for(i=1;u+9*i<64 && (u+1)%8!=0 && !mapa.contains(u+9*i);++i)
            intToButton(u+9*i)->setEnabled(true);
        if(u+9*i<64 && (u+1)%8!=0 && mapa[u+9*i]->Boja()!=f->Boja())
            intToButton(u+9*i)->setEnabled(true);

        for(i=1;u+7*i<64 && u%8!=0 && !mapa.contains(u+7*i);++i)
            intToButton(u+7*i)->setEnabled(true);
        if(u+7*i<64 && u%8!=0 && mapa[u+7*i]->Boja()!=f->Boja())
            intToButton(u+7*i)->setEnabled(true);
    }
    else if(f->Oznaka()==QOznaka::kraljica){
        int i;
       ...
       QBoja b=QBoja::bela;
        ...
        if(poljeNapadnuto(u,b))
            intToButton(u)->setEnabled(false);
...
//        mapa.remove(u);
        int i=1;
        if((u+i)%8!=0 && !poljeNapadnuto(u+1,b) && !mapa.contains(u+i))
            return true;
        if((u+i)%8!=0 && !poljeNapadnuto(u+1,b) && mapa[u+i]->Boja()!=f->Boja())
            return true;

        if((1+u-i)%8!=0 && !poljeNapadnuto(u-1,b) && !mapa.contains(u-i))
            return true;
        if((1+u-i)%8!=0 && !poljeNapadnuto(u-1,b) && mapa[u-i]->Boja()!=f->Boja())
            return true;

        if(u+8*i<64 && !poljeNapadnuto(u+8,b) && !mapa.contains(u+8*i))
            return true;
        if(u+8*i<64 && !poljeNapadnuto(u+8,b) && mapa[u+8*i]->Boja()!=f->Boja())
            return true;

        if(u+1-8*i>0 && !poljeNapadnuto(u-8,b) && !mapa.contains(u-8*i))
            return true;
        if(u+1-8*i>0 && !poljeNapadnuto(u-8,b) && mapa[u-8*i]->Boja()!=f->Boja())
            return true;


        if(u%8!=0 && u-9>-1 && !poljeNapadnuto(u-9,b) && !mapa.contains(u-9))
            return true;
        if(u%8!=0 && u-9>-1 && !poljeNapadnuto(u-9,b) && mapa[u-9]->Boja()!=f->Boja())
            return true;

        if((u+i)%8!=0 && u-7>-1 && !poljeNapadnuto(u-7,b) && !mapa.contains(u-7))
            return true;
        if((u+i)%8!=0 && u-7>-1 && !poljeNapadnuto(u-7,b) && mapa[u-7]->Boja()!=f->Boja())
            return true;

        if((u+i)%8!=0 && u+9<64 && !poljeNapadnuto(u+9,b) && !mapa.contains(u+9))
            return true;
        if((u+i)%8!=0 && u+9<64 && !poljeNapadnuto(u+9,b) && mapa[u+9]->Boja()!=f->Boja())
            return true;

        if(u%8!=0 && u+7<64 && !poljeNapadnuto(u+7,b) && !mapa.contains(u+7))
            return true;
        if(u%8!=0 && u+7<64 && !poljeNapadnuto(u+7,b) && mapa[u+7]->Boja()!=f->Boja())
            return true;
