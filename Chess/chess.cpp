    ...
    vreme(30)
{
    ...
    connect(tajmer,SIGNAL(timeout()),this,SLOT(showVreme()));
    tajmer->start(1000);
...
void Chess::poljaToButtons(){
    for(int i=0;i<64;++i)
        intToButton(i)->setEnabled(polja[i]);
}
void Chess::buttonsToPolja(){
    for(int i=0;i<64;++i)
    ...
    if(f->Oznaka()==QOznaka::pijun && f->Boja()==QBoja::bela){
       ...
       QBoja b=QBoja::bela;
        if(f->Boja()==QBoja::bela)
            b=QBoja::crna;
        mapa.remove(u);
        if(u+7<64 && u%8!=0 && !isPoljeNapadnuto(u+7,b) && (!mapa.contains(u+7) || mapa[u+7]->Boja()==b))
            intToButton(u+7)->setEnabled(true);
        if(u-7>-1 && (u+1)%8!=0 && !isPoljeNapadnuto(u-7,b) && (!mapa.contains(u-7) || mapa[u-7]->Boja()==b))
            intToButton(u-7)->setEnabled(true);

        if(u+8<64 && !isPoljeNapadnuto(u+8,b) && (!mapa.contains(u+8) || mapa[u+8]->Boja()==b))
            intToButton(u+8)->setEnabled(true);
        if(u-8>-1 && !isPoljeNapadnuto(u-8,b) && (!mapa.contains(u-8) || mapa[u-8]->Boja()==b))
            intToButton(u-8)->setEnabled(true);

        if(u+9<64 && (u+1)%8!=0 && !isPoljeNapadnuto(u+9,b) && (!mapa.contains(u+9) || mapa[u+9]->Boja()==b))
            intToButton(u+9)->setEnabled(true);
        if(u-9>-1 && u%8!=0 && !isPoljeNapadnuto(u-9,b) && (!mapa.contains(u-9) || mapa[u-9]->Boja()==b))
            intToButton(u-9)->setEnabled(true);

        if(u+1<64 && (u+1)%8!=0 && !isPoljeNapadnuto(u+1,b) && (!mapa.contains(u+1) || mapa[u+1]->Boja()==b))
            intToButton(u+1)->setEnabled(true);
        if(u-1>-1 && u%8!=0 && !isPoljeNapadnuto(u-1,b) && (!mapa.contains(u-1) || mapa[u-1]->Boja()==b))
            intToButton(u-1)->setEnabled(true);


        if(u==4 && isRokadaAvailable(2))
            intToButton(2)->setEnabled(true);
        if(u==4 && isRokadaAvailable(6))
            intToButton(6)->setEnabled(true);
        if(u==60 && isRokadaAvailable(58))
            intToButton(58)->setEnabled(true);
        if(u==60 && isRokadaAvailable(62))
        ...
        for(QMap<int,Figura*>::const_iterator i=mapa.cbegin();i!=mapa.cend();++i)
            if(i.value()->Boja()==QBoja::bela){
                if(i.value()->Oznaka()!=QOznaka::kralj){
                    if(isFiguraMovable(i.key()))
                        intToButton(i.key())->setEnabled(true);
                }
                else u=i.key();
            }
        if(isFiguraMovable(u))
            intToButton(u)->setEnabled(true);
    }
...
bool Chess::isFiguraMovable(int u){
    Figura* f=mapa[u];
    if(f->Oznaka()==QOznaka::pijun && f->Boja()==QBoja::bela){
        if(u>7 && !mapa.contains(u-8))
            return true;
    ...
    if(f->Oznaka()==QOznaka::kralj){
        ...
        mapa.remove(u);
        int i=1;
        if((u+i)%8!=0 && !isPoljeNapadnuto(u+1,b) && (!mapa.contains(u+i) || mapa[u+i]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if((1+u-i)%8!=0 && !isPoljeNapadnuto(u-1,b) && (!mapa.contains(u-i) || mapa[u-i]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if(u+8*i<64 && !isPoljeNapadnuto(u+8,b) && (!mapa.contains(u+8*i) || mapa[u+8*i]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if(u+1-8*i>0 && !isPoljeNapadnuto(u-8,b) && (!mapa.contains(u-8*i) || mapa[u-8*i]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if(u%8!=0 && u-9>-1 && !isPoljeNapadnuto(u-9,b) && (!mapa.contains(u-9) || mapa[u-9]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if((u+i)%8!=0 && u-7>-1 && !isPoljeNapadnuto(u-7,b) && (!mapa.contains(u-7) || mapa[u-7]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if((u+i)%8!=0 && u+9<64 && !isPoljeNapadnuto(u+9,b) && (!mapa.contains(u+9) || mapa[u+9]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }

        if(u%8!=0 && u+7<64 && !isPoljeNapadnuto(u+7,b) && (!mapa.contains(u+7) || mapa[u+7]->Boja()!=f->Boja())){
            mapa[u]=f;
            return true;
        }
...
bool Chess::isPoljeNapadnuto(int u,QBoja b){
    bool* a=new bool[64];
    for(int i=0;i<64;++i)
        a[i]=false;
    ...
    if(stanje==QStanje::beliIgrac_pocetakPoteza){
        ...
        if(u!=pKlik){
            ...
            if(isSah(QBoja::bela)){
                ...
                narusavaLiRokadu(pKlik);
                priprema();
                if(insufficientMaterial()){
                    allButtonsDisable();
                    tajmer->stop();
                    ui->label_krajIgre->setText("NERESENO");
                }
                restartVreme();
                ui->label_opisZaVreme->setText("CRNI");
        ...
        if(u!=pKlik){
            ...
            if(isSah(QBoja::crna)){
                ...
                restartVreme();
                ui->label_opisZaVreme->setText("BELI");
            }
        }
        else{
            stanje=QStanje::crniIgrac_pocetakPoteza;
...
bool Chess::isSah(QBoja b){
    ...
    if(i==mapa.end())
        qFatal("isSah");
    return isPoljeNapadnuto(i.key(),nb);
}
void Chess::narusavaLiRokadu(int u){
    switch (u) {
    case 0:
...
bool Chess::isRokadaAvailable(int u){
    switch (u) {
    case 2:
        return crniKralj && queensideCrniTop
                && !mapa.contains(1) && !mapa.contains(2) && !mapa.contains(3)
                && !isPoljeNapadnuto(1,QBoja::bela) && !isPoljeNapadnuto(2,QBoja::bela) && !isPoljeNapadnuto(3,QBoja::bela)
                && !isPoljeNapadnuto(0,QBoja::bela) && !isPoljeNapadnuto(4,QBoja::bela);
    case 6:
        return crniKralj && kingsideCrniTop
                && !mapa.contains(5) && !mapa.contains(6)
                && !isPoljeNapadnuto(4,QBoja::bela) && !isPoljeNapadnuto(5,QBoja::bela) && !isPoljeNapadnuto(6,QBoja::bela)
                && !isPoljeNapadnuto(7,QBoja::bela);
    case 58:
        return beliKralj && queensideBeliTop
                && !mapa.contains(57) && !mapa.contains(58) && !mapa.contains(59)
                && !isPoljeNapadnuto(56,QBoja::crna) && !isPoljeNapadnuto(57,QBoja::crna) && !isPoljeNapadnuto(58,QBoja::crna)
                && !isPoljeNapadnuto(59,QBoja::crna) && !isPoljeNapadnuto(60,QBoja::crna);
    case 62:
        return beliKralj && kingsideBeliTop
                && !mapa.contains(61) && !mapa.contains(62)
                && !isPoljeNapadnuto(60,QBoja::crna) && !isPoljeNapadnuto(61,QBoja::crna) && !isPoljeNapadnuto(62,QBoja::crna)
                && !isPoljeNapadnuto(63,QBoja::crna);
    }
    return false;
}
...
void Chess::krajIgre(){
    ui->label_pKlik->setText("");
    ui->label_vreme->setText("");
    ui->label_zaPKlik->setText("");
    ui->label_opisZaVreme->setText("");
    allButtonsDisable();
    if(stanje==QStanje::beliIgrac_pocetakPoteza || stanje==QStanje::beliIgrac_krajPoteza)
        ui->label_krajIgre->setText("Pobednik je CRNI");
    else ui->label_krajIgre->setText("Pobednik je BELI");
}
void Chess::showVreme(){
    if(vreme==0){
        krajIgre();
    }
    else{
        ui->label_vreme->setText(QString::number(vreme--));
    }
}
void Chess::restartVreme(){
    vreme=30;
}
